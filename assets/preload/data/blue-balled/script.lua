...
function opponentNoteHit(id, direction, noteType, isSustainNote)
    ...
    -- undo button (upscroll) not there yet -- At least it removes the splashes: looks better
    if mustHitSection == false and getPropertyFromClass('ClientPrefs', 'downscroll') == false and undo == true then
        if direction == 0 and isSustainNote == false then
            makeAnimatedLuaSprite('splashLEFTUD', 'noteSplashesLEFT', 340, 223)
            --setObjectCamera('splashLEFTUD', 'hud')
            addAnimationByPrefix('splashLEFTUD', 'toTheLeft', 'noteSplashes 000', 24, false)
            addLuaSprite('splashLEFTUD', true)
            runTimer('splashGoneLeftUndo', 0.2, 1)
        end
        if direction == 1 and isSustainNote == false then
            makeAnimatedLuaSprite('splashDOWNUD', 'noteSplashesDOWN', 465, 223)
            --setObjectCamera('splashDOWNUD', 'hud')
            addAnimationByPrefix('splashDOWNUD', 'toTheDown', 'noteSplashes 000', 24, false)
            addLuaSprite('splashDOWNUD', true)
            runTimer('splashGoneDownUndo', 0.2, 1)
        end
        if direction == 2 and isSustainNote == false then
            makeAnimatedLuaSprite('splashUPUD', 'noteSplashesUP', 575, 223)
            --setObjectCamera('splashUPUD', 'hud')
            addAnimationByPrefix('splashUPUD', 'toTheUp', 'noteSplashes 000', 24, false)
            addLuaSprite('splashUPUD', true)
            runTimer('splashGoneUpUndo', 0.2, 1)
        end
        if direction == 3 and isSustainNote == false then
            makeAnimatedLuaSprite('splashRIGHTUD', 'noteSplashesRIGHT', 684, 223)
            --setObjectCamera('splashRIGHTUD', 'hud')
            addAnimationByPrefix('splashRIGHTUD', 'toTheRight', 'noteSplashes 000', 24, false)
            addLuaSprite('splashRIGHTUD', true)
            runTimer('splashGoneRightUndo', 0.2, 1)
        end
    end

    -- undo button (downscroll) not there yet -- At least it removes the splashes: looks better
...
function onTimerCompleted(tag, loops, loopsLeft)
    ...
    -- undoed notesplashes timers (don't do shit for now)
    if tag == 'splashGoneLeftUndo' then
		removeLuaSprite('splashLEFTUD', false)
	end
	if tag == 'splashGoneRightUndo' then
		removeLuaSprite('splashRIGHTUD', false)
	end
	if tag == 'splashGoneUpUndo' then
		removeLuaSprite('splashUPUD', false)
	end
	if tag == 'splashGoneDownUndo' then
		removeLuaSprite('splashDOWNUD', false)
	end
